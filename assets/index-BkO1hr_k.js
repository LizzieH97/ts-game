...
flakes of rust.`,summary:"Bell: Rusty, but still works. Has only just been placed there on the deck, but has been shedding small rust flakes.",result:"Nope, not the bell! It would've made too much noise. Olivia heard some clanging from the toilet, but that was just George messing with the pipes in an act of sabotage towards the Captain."},l=({text:e,clues:o,speed:s=50,scrollAt:a=20})=>{const t=e.split(`
`);let n=0,r=0;const T=()=>{let q="",E=Math.max(0,n-a);for(;E<n;)q+=t[E++]+"<br />";o.innerHTML=q+t[n].substring(0,r),r++<t[n].length?setTimeout(T,s):(r=0,n++,n<t.length&&setTimeout(T,10))};T()},oe=document.querySelector(".background"),ae=document.querySelector(".board"),ne=document.querySelectorAll(".board__piece"),se=document.querySelectorAll(".board__piece--person"),ie=document.querySelectorAll(".board__piece--tool"),H=document.querySelector(".openingScreen"),A=document.querySelector(".openingScreen__btn"),b=document.querySelector(".cluesOverlay"),i=document.querySelector(".cluesOverlay__text"),C=document.querySelector(".cluesOverlay__btnbox"),u=document.querySelectorAll(".cluesOverlay__btnbox--btn"),N=document.querySelector(".cluesOverlay__close"),S=document.querySelector(".notebookOverlay"),B=document.querySelector(".header__notebook"),m=document.querySelector(".header__captain"),w=document.querySelector(".notebookOverlay__bulletPts"),D=document.querySelector(".notebookOverlay__addNote"),g=document.querySelector(".notebookOverlay__addNote--input"),y=document.querySelector(".notebookOverlay__close"),I=document.querySelector(".captainOverlay"),W=document.querySelector(".captainOverlay__close"),f=document.querySelector(".captainOverlay__initial"),re=document.querySelectorAll(".captainOverlay__initial--btn"),P=document.querySelector(".captainOverlay__info--close"),x=document.querySelector(".captainOverlay__final--close"),k=document.querySelector(".captainOverlay__info"),v=document.querySelector(".captainOverlay__final"),M=document.querySelector(".accusationForm__suspects"),R=document.querySelector(".accusationForm__weapons"),p=document.querySelector(".accusationForm"),h=document.querySelector(".captainOverlay__final--person-result"),d=document.querySelector(".captainOverlay__final--weapon-result"),_=document.querySelector(".captainOverlay__final--text"),c=document.querySelector(".captainOverlay__final--end-game"),O={bucket:K,chair:J,rope:Q,chair2:X,extinguisher:Z,barrel:ee,bell:te},L={angela:F,george:G,liam:Y,olivia:V,rachel:z,tom:U,tessa:$};if(!ae||!ne||!oe||!A||!H||!i||!N||!b||!C||!S||!B||!w||!D||!g||!y||!m||!I||!W||!f||!k||!P||!x||!v||!M||!R||!p||!h||!d||!_||!c)throw new Error("Something went wrong! ");const le=e=>{e.preventDefault(),H.style.display="none"},he=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");b.style.display="flex";const s=e.currentTarget.id,a=L[s];a&&(i.innerText="",l({text:a.initial,clues:i}),u[0].innerText="Where were you at 3pm?",u[1].innerText="Who do you think it was?")},de=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");b.style.display="flex";const s=e.currentTarget.id,a=O[s];a&&l({text:a.initial,clues:i}),u[0].innerText="Inspect closer",u[1].innerText="Look underneath"},ce=e=>{const o=e.currentTarget;if(!o.parentElement)throw new Error("whoops! something went wrong :/");const s=o.parentElement.id,a=L[s],t=O[s];if(o.id==="btn-1"&&(i.innerText="",l(a?{text:a.alibi,clues:i}:{text:t.closer,clues:i})),o.id==="btn-2"&&(i.innerText="",l(a?{text:a.accusation,clues:i}:{text:t.under,clues:i})),o.id==="post-to-notepad"){const n=a?a.summary:t.summary,r=document.createElement("li");r.innerHTML=`${n} <button class="deleteText">Delete</button>`,w.appendChild(r),g.value=""}},ue=e=>{e.preventDefault(),i.innerText="",C.id="",b.style.display="none"},pe=e=>{e.preventDefault(),S.style.display="flex"},me=e=>{e.preventDefault();const o=g.value;if(!o)return;const s=document.createElement("li");s.innerHTML=`${o} <button class="deleteText" >Delete</button>`,w.appendChild(s),g.value=""},ye=e=>{e.preventDefault(),S.style.display="none"},ge=e=>{e.preventDefault(),I.style.display="grid",f.style.display="grid"},be=e=>{e.preventDefault(),I.style.display="none",k.style.display="none",v.style.display="none",p.style.display="none"},we=e=>{if(e.preventDefault(),e.target){_.style.display="flex";const o=e.currentTarget;f.style.display="none",o.id==="moreInfo"?k.style.display="flex":(v.style.display="flex",p.style.display="flex")}},j=e=>{e.preventDefault(),k.style.display="none",v.style.display="none",f.style.display="grid",h.innerText="",d.innerText=""},fe=e=>{e.preventDefault(),p.style.display="none",_.style.display="none",d.style.display="flex",h.style.display="flex";const o=M.value,s=R.value,a=L[o],t=O[s];o==="tom"&&s==="extinguisher"?(h.innerHTML=a.result,d.innerText=t.result,x.style.display="none",c.style.display="flex"):(h.innerText=a.result,d.innerText=t.result)};se.forEach(e=>e.addEventListener("click",he));ie.forEach(e=>e.addEventListener("click",de));A.addEventListener("click",le);N.addEventListener("click",ue);u.forEach(e=>{e.addEventListener("click",ce)});B.addEventListener("click",pe);D.addEventListener("submit",me);y==null||y.addEventListener("click",ye);w.addEventListener("click",e=>{const o=e.target;if(o.classList.contains("deleteText")){const s=o.closest("li");s&&s.remove()}});m==null||m.addEventListener("click",ge);W.addEventListener("click",be);re.forEach(e=>{e.addEventListener("click",we)});P.addEventListener("click",j);x.addEventListener("click",j);p.addEventListener("submit",fe);c==null||c.addEventListener("click",()=>{location.reload()});
