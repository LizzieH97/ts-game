...
`);let n=0,r=0;const S=()=>{let E="",H=Math.max(0,n-a);for(;H<n;)E+=o[H++]+"<br />";t.innerHTML=E+o[n].substring(0,r),r++<o[n].length?setTimeout(S,s):(r=0,n++,n<o.length&&setTimeout(S,10))};S()},oe=document.querySelector(".background"),ae=document.querySelector(".board"),ne=document.querySelectorAll(".board__piece"),se=document.querySelectorAll(".board__piece--person"),ie=document.querySelectorAll(".board__piece--tool"),A=document.querySelector(".openingScreen"),C=document.querySelector(".openingScreen__btn"),b=document.querySelector(".cluesOverlay"),i=document.querySelector(".cluesOverlay__text"),re=document.querySelector(".cluesOverlay__img"),w=document.querySelector(".cluesOverlay__btnbox"),u=document.querySelectorAll(".cluesOverlay__btnbox--btn"),N=document.querySelector(".cluesOverlay__close"),I=document.querySelector(".notebookOverlay"),B=document.querySelector(".header__notebook"),p=document.querySelector(".header__captain"),f=document.querySelector(".notebookOverlay__bulletPts"),D=document.querySelector(".notebookOverlay__addNote"),g=document.querySelector(".notebookOverlay__addNote--input"),y=document.querySelector(".notebookOverlay__close"),x=document.querySelector(".captainOverlay"),W=document.querySelector(".captainOverlay__close"),k=document.querySelector(".captainOverlay__initial"),le=document.querySelectorAll(".captainOverlay__initial--btn"),P=document.querySelector(".captainOverlay__info--close"),_=document.querySelector(".captainOverlay__final--close"),v=document.querySelector(".captainOverlay__info"),T=document.querySelector(".captainOverlay__final"),M=document.querySelector(".accusationForm__suspects"),R=document.querySelector(".accusationForm__weapons"),m=document.querySelector(".accusationForm"),h=document.querySelector(".captainOverlay__final--person-result"),d=document.querySelector(".captainOverlay__final--weapon-result"),O=document.querySelector(".captainOverlay__final--text"),c=document.querySelector(".captainOverlay__final--end-game"),L={bucket:K,chair:J,rope:Q,chair2:X,extinguisher:Z,barrel:ee,bell:te},q={angela:F,george:G,liam:Y,olivia:V,rachel:z,tom:U,tessa:$};if(!ae||!ne||!oe||!C||!A||!i||!N||!b||!re||!w||!I||!B||!f||!D||!g||!y||!p||!x||!W||!k||!v||!P||!_||!T||!M||!R||!m||!h||!d||!O||!c)throw new Error("Something went wrong! ");const he=e=>{e.preventDefault(),A.style.display="none"},de=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");b.style.display="flex";const t=e.currentTarget,s=t.id;w.id=t.id;const a=q[s];a&&(i.innerText="",l({text:a.initial,clues:i}),u[0].innerText="Where were you at 3pm?",u[1].innerText="Who do you think it was?")},ce=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");b.style.display="flex";const t=e.currentTarget,s=t.id;w.id=t.id;const a=L[s];a&&l({text:a.initial,clues:i}),u[0].innerText="Inspect closer",u[1].innerText="Look underneath"},ue=e=>{const t=e.currentTarget;if(!t.parentElement)throw new Error("whoops! something went wrong :/");const s=t.parentElement.id,a=q[s],o=L[s];if(t.id==="btn-1"&&(i.innerText="",l(a?{text:a.alibi,clues:i}:{text:o.closer,clues:i})),t.id==="btn-2"&&(i.innerText="",l(a?{text:a.accusation,clues:i}:{text:o.under,clues:i})),t.id==="post-to-notepad"){const n=a?a.summary:o.summary,r=document.createElement("li");r.innerHTML=`${n} <button class="deleteText">Delete</button>`,f.appendChild(r),g.value=""}},me=e=>{e.preventDefault(),i.innerText="",w.id="",b.style.display="none"},pe=e=>{e.preventDefault(),I.style.display="flex"},ye=e=>{e.preventDefault();const t=g.value;if(!t)return;const s=document.createElement("li");s.innerHTML=`${t} <button class="deleteText" >Delete</button>`,f.appendChild(s),g.value=""},ge=e=>{e.preventDefault(),I.style.display="none"},be=e=>{e.preventDefault(),x.style.display="grid",k.style.display="grid"},we=e=>{e.preventDefault(),x.style.display="none",v.style.display="none",T.style.display="none",m.style.display="none"},fe=e=>{if(e.preventDefault(),e.target){O.style.display="flex";const t=e.currentTarget;k.style.display="none",t.id==="moreInfo"?v.style.display="flex":(T.style.display="flex",m.style.display="flex")}},j=e=>{e.preventDefault(),v.style.display="none",T.style.display="none",k.style.display="grid",h.innerText="",d.innerText=""},ke=e=>{e.preventDefault(),m.style.display="none",O.style.display="none",d.style.display="flex",h.style.display="flex";const t=M.value,s=R.value,a=q[t],o=L[s];t==="tom"&&s==="extinguisher"?(h.innerHTML=a.result,d.innerText=o.result,_.style.display="none",c.style.display="flex"):(h.innerText=a.result,d.innerText=o.result)};se.forEach(e=>e.addEventListener("click",de));ie.forEach(e=>e.addEventListener("click",ce));C.addEventListener("click",he);N.addEventListener("click",me);u.forEach(e=>{e.addEventListener("click",ue)});B.addEventListener("click",pe);D.addEventListener("submit",ye);y==null||y.addEventListener("click",ge);f.addEventListener("click",e=>{const t=e.target;if(t.classList.contains("deleteText")){const s=t.closest("li");s&&s.remove()}});p==null||p.addEventListener("click",be);W.addEventListener("click",we);le.forEach(e=>{e.addEventListener("click",fe)});P.addEventListener("click",j);_.addEventListener("click",j);m.addEventListener("submit",ke);c==null||c.addEventListener("click",()=>{location.reload()});
