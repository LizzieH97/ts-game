...
new bucket...`,summary:"Bucket: Brand new, label still on it, no marks or dents. It is leaking sand out of the bottom.",result:"Nope, the bucket wasn't used! Someone, i.e. George, may have bought it because it was faulty though..."},Q={initial:`The body is lying next to the deck
...
the sun.`,summary:"Rope: Giant, heavily pile which is stuck together due to long exposure to sea air and water. One part has been exposed, showing the unexposed inside sections. Some blood splashes.",result:"Nope, it wasn't the rope! The reason it had been moved was because the murderer had leant against it, feeling overwhelmed after the murder."},Z={initial:`Similar to the other chair, this one
...
damage to the deck.`,summary:"Barrel: Empty, but looks like it was leaking water before. Several small dents and a few specks of rust at the top.",result:"There's literally no evidence that it's the barrel. How would someone be able to hit someone over the head with it. The only 'crime' the barrel has committed is being leaky."},oe={initial:`A rusted bell lays on the floor, the
...
`);let n=0,r=0;const I=()=>{let H="",A=Math.max(0,n-a);for(;A<n;)H+=o[A++]+"<br />";t.innerHTML=H+o[n].substring(0,r),r++<o[n].length?setTimeout(I,s):(r=0,n++,n<o.length&&setTimeout(I,10))};I()},ae=document.querySelector(".background"),ne=document.querySelector(".board"),se=document.querySelectorAll(".board__piece"),ie=document.querySelectorAll(".board__piece--person"),re=document.querySelectorAll(".board__piece--tool"),C=document.querySelector(".openingScreen"),N=document.querySelector(".openingScreen__btn"),f=document.querySelector(".cluesOverlay"),i=document.querySelector(".cluesOverlay__text"),g=document.querySelector(".cluesOverlay__img"),w=document.querySelector(".cluesOverlay__btnbox"),u=document.querySelectorAll(".cluesOverlay__btnbox--btn"),B=document.querySelector(".cluesOverlay__close"),x=document.querySelector(".notebookOverlay"),D=document.querySelector(".header__notebook"),m=document.querySelector(".header__captain"),k=document.querySelector(".notebookOverlay__bulletPts"),W=document.querySelector(".notebookOverlay__addNote"),b=document.querySelector(".notebookOverlay__addNote--input"),y=document.querySelector(".notebookOverlay__close"),_=document.querySelector(".captainOverlay"),P=document.querySelector(".captainOverlay__close"),v=document.querySelector(".captainOverlay__initial"),le=document.querySelectorAll(".captainOverlay__initial--btn"),M=document.querySelector(".captainOverlay__info--close"),O=document.querySelector(".captainOverlay__final--close"),T=document.querySelector(".captainOverlay__info"),S=document.querySelector(".captainOverlay__final"),R=document.querySelector(".accusationForm__suspects"),j=document.querySelector(".accusationForm__weapons"),p=document.querySelector(".accusationForm"),h=document.querySelector(".captainOverlay__final--person-result"),d=document.querySelector(".captainOverlay__final--weapon-result"),L=document.querySelector(".captainOverlay__final--text"),c=document.querySelector(".captainOverlay__final--end-game"),q={bucket:J,chair:Q,rope:X,chair2:Z,extinguisher:ee,barrel:te,bell:oe},E={angela:G,george:Y,liam:V,olivia:$,rachel:z,tom:U,tessa:K};if(!ne||!se||!ae||!N||!C||!i||!B||!f||!g||!w||!x||!D||!k||!W||!b||!y||!m||!_||!P||!v||!T||!M||!O||!S||!R||!j||!p||!h||!d||!L||!c)throw new Error("Something went wrong! ");const he=e=>{e.preventDefault(),C.style.display="none"},de=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");f.style.display="flex";const t=e.currentTarget,s=t.id;w.id=t.id,g.src=`../public/${t.id}.png`;const a=E[s];a&&(i.innerText="",l({text:a.initial,clues:i}),u[0].innerText="Where were you at 3pm?",u[1].innerText="Who do you think it was?")},ce=e=>{if(!i||!e.currentTarget)throw new Error("Something went wrong! ");f.style.display="flex";const t=e.currentTarget,s=t.id;w.id=t.id,t.id==="chair2"?g.src="../public/chair.png":g.src=`../public/${t.id}.png`;const a=q[s];a&&l({text:a.initial,clues:i}),u[0].innerText="Inspect closer",u[1].innerText="Look underneath"},ue=e=>{const t=e.currentTarget;if(!t.parentElement)throw new Error("whoops! something went wrong :/");const s=t.parentElement.id,a=E[s],o=q[s];if(t.id==="btn-1"&&(i.innerText="",l(a?{text:a.alibi,clues:i}:{text:o.closer,clues:i})),t.id==="btn-2"&&(i.innerText="",l(a?{text:a.accusation,clues:i}:{text:o.under,clues:i})),t.id==="post-to-notepad"){const n=a?a.summary:o.summary,r=document.createElement("li");r.innerHTML=`${n} <button class="deleteText">Delete</button>`,k.appendChild(r),b.value=""}},pe=e=>{e.preventDefault(),i.innerText="",w.id="",f.style.display="none"},me=e=>{e.preventDefault(),x.style.display="flex"},ye=e=>{e.preventDefault();const t=b.value;if(!t)return;const s=document.createElement("li");s.innerHTML=`${t} <button class="deleteText" >Delete</button>`,k.appendChild(s),b.value=""},ge=e=>{e.preventDefault(),x.style.display="none"},be=e=>{e.preventDefault(),_.style.display="grid",v.style.display="grid"},fe=e=>{e.preventDefault(),_.style.display="none",T.style.display="none",S.style.display="none",p.style.display="none"},we=e=>{if(e.preventDefault(),e.target){L.style.display="flex";const t=e.currentTarget;v.style.display="none",t.id==="moreInfo"?T.style.display="flex":(S.style.display="flex",p.style.display="flex")}},F=e=>{e.preventDefault(),T.style.display="none",S.style.display="none",v.style.display="grid",h.innerText="",d.innerText=""},ke=e=>{e.preventDefault(),p.style.display="none",L.style.display="none",d.style.display="flex",h.style.display="flex";const t=R.value,s=j.value,a=E[t],o=q[s];t==="tom"&&s==="extinguisher"?(h.innerHTML=a.result,d.innerText=o.result,O.style.display="none",c.style.display="flex"):(h.innerText=a.result,d.innerText=o.result)};ie.forEach(e=>e.addEventListener("click",de));re.forEach(e=>e.addEventListener("click",ce));N.addEventListener("click",he);B.addEventListener("click",pe);u.forEach(e=>{e.addEventListener("click",ue)});D.addEventListener("click",me);W.addEventListener("submit",ye);y==null||y.addEventListener("click",ge);k.addEventListener("click",e=>{const t=e.target;if(t.classList.contains("deleteText")){const s=t.closest("li");s&&s.remove()}});m==null||m.addEventListener("click",be);P.addEventListener("click",fe);le.forEach(e=>{e.addEventListener("click",we)});M.addEventListener("click",F);O.addEventListener("click",F);p.addEventListener("submit",ke);c==null||c.addEventListener("click",()=>{location.reload()});
